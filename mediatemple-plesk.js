...
var steps = [
    function() { // Log in
        page.evaluate(function() {
            ...
            document.querySelector('input#loginSection-username').value = window.callPhantom('username');
            document.querySelector('input#loginSection-password').value = window.callPhantom('password');
            //document.querySelector('input[name="ctl00$ContentPlaceHolder1$btnLogin"]').submit();
            document.querySelector('form[action="https://ykyq-qkpr.accessdomain.com:8443/relay"]').submit();
            console.log('Logging in...');
        });
    },
    function() { // Click Volunteer Button
        page.evaluate(function() {
            ...
            var $volunteerBtn = document.querySelector('a#buttonAddSubDomain');

            if (!$volunteerBtn) {
                return window.callPhantom('fatal-error', 'Unable to find terms acceptance button');
            }

            //fireClick($volunteerBtn);
            console.log($volunteerBtn);
            console.log('Click Volunteer Button...');
        });
    },
    function() { // main volunteer dashboard
        page.evaluate(function() {
            function fireClick(el) {
                var ev = document.createEvent("MouseEvents");
                ev.initEvent("click", true, true);
            ...
            console.log( jQuery("#clsVolunteerCommitement h2").text() );
